...
public class TestInputBuffering {
    ...
    public static void main(String[] args) {
        ...
        try {
            TestInputBuffering test = new TestInputBuffering();
    ...
    private void inicializarPruebas() throws Exception {
        ...
        Set<Integer> keysTemp = (Set<Integer>) keysField.get(controller);
        keysPressed = keysTemp;

        Field keyTimeField = GameController.class.getDeclaredField("keyPressTime");
        keyTimeField.setAccessible(true);

        this.prueba("Inicialización", game != null && board != null && keysPressed != null);
    ...
    private void pruebaMovimientoHelados() throws Exception {
        ...
        Position posActual1 = new Position(iceCream.getPosition());

        boolean intento_derecha = game.moveIceCream(Direction.RIGHT);
        Position posActual2 = new Position(iceCream.getPosition());

        boolean intento_abajo = game.moveIceCream(Direction.DOWN);
        Position posActual3 = new Position(iceCream.getPosition());

        boolean intento_izquierda = game.moveIceCream(Direction.LEFT);
        Position posActual4 = new Position(iceCream.getPosition());
    ...
    private void pruebaGeneracionRupturaHielo() throws Exception {
        ...
        // Simular creación de bloque de hielo (Q)
        game.createIceBlock();
