...
public class PresentationController {
    ...
    private void mostrarConfiguracionFrutas() {
        ...
        fruitConfigMenu.setOnConfirmClick(() -> {
            ...
            // >>> MOSTRAR MENÚ DE OBSTÁCULOS AQUÍ <<<
            ObstaculosConfigurationMenu obstaculosMenu = new ObstaculosConfigurationMenu();
        
            obstaculosMenu.setOnConfirmClick(() -> {
                Map<String, Integer> obstacleConfig = obstaculosMenu.getObstacleConfiguration();
            
                // Pasa los datos a un lugar global si los necesitas después
                int cantidadFogatas = obstacleConfig.getOrDefault("Fogata", 0);
                int cantidadBaldosas = obstacleConfig.getOrDefault("Baldosa Caliente", 0);
            
                // Ejemplo: puedes llamar tu método para iniciar el juego, 
                // pero asegurándote de pasar estos parámetros al Game (o GameController)
                iniciarJuegoSegunModoConObstaculos(selectedIceCream, cantidadFogatas, cantidadBaldosas);
            
                obstaculosMenu.setVisible(false);
            });
        
            obstaculosMenu.setOnBackClick(() -> {
                obstaculosMenu.setVisible(false);
                fruitConfigMenu.setVisible(true);
            });
        
            obstaculosMenu.setVisible(true);
        });
    ...
    /**
     * Inicia el juego según el modo seleccionado, pasando la cantidad de fogatas y baldosas calientes
     */
    private void iniciarJuegoSegunModoConObstaculos(String helado, int cantidadFogatas, int cantidadBaldosas) {
        if (selectedGameMode == GameMode.PVM) {
            iniciarJuegoPVMConFogatas(helado, cantidadFogatas, cantidadBaldosas);
        } else if (selectedGameMode == GameMode.PVP) {
            if (selectedPVPMode == PVPMode.ICE_CREAM_COOPERATIVE) {
                iniciarJuegoCooperativoConFogatas(helado, selectedSecondIceCream, cantidadFogatas, cantidadBaldosas);
            } else if (selectedPVPMode == PVPMode.ICE_CREAM_VS_MONSTER) {
                if (selectedMonster != null) {
                    iniciarJuegoVSMonstruoConFogatas(helado, selectedMonster, selectedIceCreamAIStrategy, cantidadFogatas, cantidadBaldosas);
                } else {
                    System.err.println("❌ Error: No se ha seleccionado monstruo para modo PVP Vs Monstruo");
                }
            }
        } else if (selectedGameMode == GameMode.MVM) {
            iniciarJuegoMVMConFogatas(cantidadFogatas, cantidadBaldosas);
        }
    }

    private void iniciarJuegoCooperativoConFogatas(String helado1, String helado2, int cantidadFogatas, int cantidadBaldosas) {
        cleanupBeforeNewGame();
        pvp.setVisible(false);
        selectMonster.setVisible(false);
        selectPVPMode.setVisible(false);
        ...
        gameController = new GameController(selectedGameMode, helado1, helado2, null, selectedEnemyConfig, selectedFruitConfig);

        gameController.getGame().setMostrarFogatas(cantidadFogatas > 0);
        gameController.getGame().setCantidadFogatas(cantidadFogatas);
        gameController.getGame().setCantidadBaldosasCalientes(cantidadBaldosas);
    ...
    private void iniciarJuegoPVMConFogatas(String helado, int cantidadFogatas, int cantidadBaldosas) {
        cleanupBeforeNewGame();
        pvp.setVisible(false);
        selectMonster.setVisible(false);
        selectPVPMode.setVisible(false);
        ...
        gameController = new GameController(selectedGameMode, helado, null, null, selectedEnemyConfig, selectedFruitConfig);
    ...
    private void iniciarJuegoVSMonstruoConFogatas(String helado, String monstruo, String aiStrategy, int cantidadFogatas, int cantidadBaldosas) {
        cleanupBeforeNewGame();
        pvp.setVisible(false);
        selectIceCreamAI.setVisible(false);
        selectMonster.setVisible(false);
        ...
        gameController = new GameController(selectedGameMode, helado, null, monstruo, selectedEnemyConfig, selectedFruitConfig);
        if (aiStrategy != null && !aiStrategy.isEmpty()) {
            gameController.setIceCreamAIStrategy(aiStrategy);
        }
        gameController.getGame().setMostrarFogatas(cantidadFogatas > 0);
    ...
    private void iniciarJuegoMVMConFogatas(int cantidadFogatas, int cantidadBaldosas) {
        cleanupBeforeNewGame();
        pvp.setVisible(false);
        selectMonster.setVisible(false);
        selectPVPMode.setVisible(false);
        ...
        gameController = new GameController(selectedGameMode, selectedIceCream, null, null, selectedEnemyConfig, selectedFruitConfig);
